...
class PageScrap:
    ...
    def scrape_site(url):
        ...
        for row in rating_table.find_all('tr', class_='a-histogram-row a-align-center'):
            percentage_element = row.find('td', class_='a-text-right a-nowrap a-nowrap').find('a')
        ...
        #free delivery
        free_delivery_element = soup.find('div', id="mir-layout-DELIVERY_BLOCK-slot-PRIMARY_DELIVERY_MESSAGE_LARGE")
        free_delivery = False
        if(free_delivery_element):
            free_delivery = free_delivery_element.text.strip().startswith('FREE delivery')
        ...
        #amazon choice
        amazon_choice_element = soup.find('span', class_='a-size-small aok-float-left ac-badge-rectangle')
        amazon_choice = False
        if(amazon_choice_element): amazon_choice = True

        #limited time deal
        deal_element = soup.find('div', id='dealBadge_feature_div')
        deal = False
        if(deal_element): deal = True
        ...
        if product_details_element:
            ...
            for char in product_details_element_text:
                if PageScrap.is_printable_ascii(char):
                    if char.isspace():
                        ...
                        tokens.append(char)
                        current_token = ""
                        in_multiple_spaces = False  # Reset multiple space flag after colon
                    else:
                        # Regular character, add to current token
                        current_token += char
                        in_multiple_spaces = False  # Reset multiple space flag for regular characters
            ...
            for i in range(1, len(tokens)):  # Iterate using indexes
                ...
                if token == ':':  # Check if token is colon
                    identifier = tokens[i-1].strip()  # Extract identifier (remove colon)
                    if i + 1 < len(tokens):
                        if(identifier == 'Available'):
                            date_first_available = tokens[i+1] + " " + tokens[i+2] + " " + tokens[i+3]
                        elif(identifier == 'ASIN'):
                            asin = tokens[i+1]


        # rank in category
        details_list = product_details_element.find_next_sibling('ul', class_='a-unordered-list a-nostyle a-vertical a-spacing-none detail-bullet-list')
