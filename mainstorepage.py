    ...
    async def product_urls(self, url, max_retries = 13):
        ...
        for retry in range(max_retries):
            try:
                ...
                card_contents = [f"""https://www.amazon.com{product.select_one(self.hyperlink).get('href')}""" for product in soup.select(self.main_content)]
                #url_lists.append(card_contents)
                if(card_contents): return card_contents
            except Exception as e:
